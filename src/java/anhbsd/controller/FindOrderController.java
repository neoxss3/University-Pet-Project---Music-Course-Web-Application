...
public class FindOrderController extends HttpServlet {
    private static final String ERROR="tracking.jsp";
 //   private static final String showSearchResult="ShowSearchResultServlet";
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        response.setContentType("text/html;charset=UTF-8");
        String url=ERROR;
        String value=request.getParameter("ordid");
        
        try{
          if(!value.isEmpty()){
              ProductDAO dao=new ProductDAO();
              dao.searchorder(value);
              List<OrderDTO> result=dao.getOrderlist();
              request.setAttribute("ORDER_RESULT", result);
              url=ERROR;
